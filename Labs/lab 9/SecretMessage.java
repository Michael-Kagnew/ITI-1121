...
public class SecretMessage {
    ...
    public static void encrypt( String inputFilem, String outputFile, int key ) throws IOException, FileNotFoundException {
        ...
        try{
             ...
             int num = 0;
             while( (num = input.read()) != -1){ //THIS AFFECTS WHAT IS STORED IN INPUT, will eventually reach no characters
                out.write((char) (num+key));

             }
        }
        finally{
            if(input != null) { //Need to write this because if input is null, then will crash when calling close
            input.close();   
            }
