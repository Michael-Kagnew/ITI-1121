...
public class ArrayQueue<E> implements Queue<E> {
    ...
    public void enqueue( E elem ) {
        ...
        // pre-condition: ???
        if(isFull()){
            throw new IllegalStateException("can't put in a full queue");
        }
    ...
    public E dequeue() {
        ...
        // pre-condition: ???
        if (isEmpty() == 0){
            throw new IllegalStateException("Can't remove element from empty");
        }

        E saved = elems[ front ];
        elems[ front ] = null; // scrubbing the memory!
   /*     if(size == 1){ //Do we need to take into account dequeing last items?
