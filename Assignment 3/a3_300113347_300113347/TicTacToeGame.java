...
public class TicTacToeGame {
	...
	*/
	...
	int counter = 0; //Keep track for square or non square number of symmetries to execute
	boolean bType = true;  //If board is a square or not
    ...
    *  String representation of the game
    ...
    public boolean hasNext(){
    	...
    	if (lines == columns && counter <8){
    		bType = true; // The board is a perfect square (technically don't need it, already set to true)
    		return true;
    ...
    public void next(){
    	...
    	// YOUR CODE HERE
    	if (!hasNext()){
    		throw new IllegalStateException("No more symmetries, must reset");
    	}
    	if (hasNext()){
    		if(bType == true){ //So board is a square
    			if ((counter> 0 && counter < 4) || (counter > 4 && counter <8)){
    				Utils.rotate(lines, columns, transformedBoard);
    			
    			} else if (counter != 0) { //have to state this because for ID board, there is no change when the first next is called
